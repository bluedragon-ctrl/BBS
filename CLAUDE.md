...
- **Scene flavor.** Every node carries an optional `node.scene = { room, pre, post }` (all optional plain strings on the node itself). `room` is the persistent place description — surfaced as the default INSPECT body on map and combat screens, and prepended to non-combat node modals. `pre` types into the log strip in italics on combat-screen entry (after `awaitLogIdle`, before the scheduler runs). `post` types into the log strip on victory and gates the screen exit on a player keypress / click — without `scene.post`, combat auto-leaves on the original timer.

  **Authoring.** `data/nodes.json` holds scene **templates**: `{ id, match: { type, layer? }, scene: { room, pre, post } }`. In templates each of `room`/`pre`/`post` may be either a string OR an array of strings. `engine/map.js#generateMap` picks the most type+layer-specific matching template per node, then `resolveSceneVariants` collapses any arrays to a single string via the run rng — so the saved `node.scene` is always flat strings (and survives suspend/load through `serializeMap`). Authoring an array of N pres × an array of M posts on a single template gives N×M possible runs of that template; multiple templates matching the same `(type, layer)` multiplies further. Use this for variety in nodes without authoring a unique template per variant.

  **Interpolation.** Scene strings pass through `formatSceneString(text, ctx)` (in `src/engine/scene.js`) at display time. Supported `{path}` shapes: `{enemies[0].name}`, `{enemies[1].name}`, ..., `{enemyCount}`, `{enemyNames}` (comma-joined), `{player.name}` (combat only), `{layer}`, `{nodeType}` (map preview only). Two context builders cover the two surfaces: `sceneContextFromCombat(combat)` uses live actor names — including any CONN-driven name corruption already applied at combat start — and is what `combat.js` uses for pre/post log lines and the inspect-default fallback. `sceneContextFromNode(node, data)` uses canonical `data.monster(id).name` lookups for the map-screen inspect preview before combat starts. Missing paths render as empty string — failure-soft.

  **Naming.** Three different shapes share the word "flavor" elsewhere — `monster.flavor` (random AI lines), `showNodeModal({ flavor })` (modal body string), and template entries here under `scene.*` — keep the names straight when reading the code. `logFlavor(text)` (in `src/log.js`) is the canonical way to emit italic scene narration; rendered with `.log-flavor` styling.
- **MUD-ish expansions** — the console is doing more narrative work as the design evolves. Future content can lean further into MUD territory without restructuring: named features inside `scene.room` that become inspectable subjects (`> inspect campfire`), exits described by text rather than glyphs, NPC chatter via the existing monster `flavor` mechanism, and verb-shaped buttons (`[L]ook`, `[T]alk`, `[U]se`) when nodes warrant them. Treat these as content/data extensions on top of `scene` — no new core systems needed.
